...
class AWBSegmentationPayment(models.Model):
	...
	# 			if rec.invoice.invoice_line_ids.product_id.product_tmpl_id.product_segmentation == "month_service":
	# 				total1 += rec.invoice_amount
	# 				self.monthly_service = total1
	# 				continue
	# 			elif rec.invoice.invoice_line_ids.product_id.product_tmpl_id.product_segmentation == "device":
	# 				total2 += rec.invoice_amount
	# 				self.device = total2
	# 				continue
	# 			elif rec.invoice.invoice_line_ids.product_id.product_tmpl_id.product_segmentation == "security_deposit":
	# 				total3 += rec.invoice_amount
	# 				self.security_deposit = total3
	# 				continue
	# 			elif rec.invoice.invoice_line_ids.product_id.product_tmpl_id.product_segmentation == "others":
	# 				total4 += rec.invoice_amount
	# 				self.others = total4
	# 				continue